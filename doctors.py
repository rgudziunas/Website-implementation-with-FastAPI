...
from auth import get_current_admin, get_authenticated_user
...
def list_doctors(db: DBSession):
    """List all doctors - Public access"""
...
def get_doctor(doctor_id: int, db: DBSession):
    """Get doctor details - Public access"""
...
def list_doctor_appointments(
    ...
    """Get doctor appointments - Authenticated users (for availability checking)"""
...
def list_doctor_services(doctor_id: int, db: DBSession):
    """Get all services that a doctor can perform - Public access"""
    doctor = db.query(models.Doctor).filter(models.Doctor.id == doctor_id).first()
    if not doctor:
        raise HTTPException(404, "Doctor not found")
...
def link_service_to_doctor(
    doctor_id: int,
    payload: LinkServiceIn,
    db: DBSession,
    current_admin: Annotated[models.Admin, Depends(get_current_admin)]
):
    """Link a service to a doctor - Admin only"""
    doctor = db.query(models.Doctor).filter(models.Doctor.id == doctor_id).first()
    if not doctor:
        raise HTTPException(404, "Doctor not found")
...
def unlink_service_from_doctor(
    doctor_id: int,
    service_id: int,
    db: DBSession,
    current_admin: Annotated[models.Admin, Depends(get_current_admin)]
):
    """Unlink a service from a doctor - Admin only"""
